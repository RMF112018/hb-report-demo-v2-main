...
export const ThreadPanel = ({ thread, task, className }: ThreadPanelProps) => {
  ...
  const handleSendMessage = () => {
    ...
    if (isTaskThread && task) {
      addTaskComment(task.id, messageData)
    } else if (thread) {
  ...
  const handleReaction = (messageId: string, emoji: string) => {
    ...
    const existingReaction = messages
      ...
      ?.reactions?.find(r => r.userId === currentUserId && r.emoji === emoji)
  ...
  const getLinkedEntityBadge = () => {
    ...
    return (
      <Badge variant="outline" className={cn('mb-4', typeColors[linkedTo.type])}>
