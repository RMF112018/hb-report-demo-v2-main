...
import { Search, MessageSquare, Users, Clock, ExternalLink, Filter, Plus } from "lucide-react"
...
export const MessageBoard = ({ className, selectedThreadId, autoCreate }: MessageBoardProps) => {
  ...
  useEffect(() => {
    if (selectedThreadId) {
      const thread = messageThreads.find((t: MessageThread) => t.id === selectedThreadId)
      if (thread) {
  ...
  const filteredThreads = useMemo(() => {
    ...
    // Filter by user
    if (filterUser !== "all") {
      threads = threads.filter((thread: MessageThread) => thread.participants.includes(filterUser))
    }

    // Filter by linked feature
    if (filterFeature !== "all") {
      threads = threads.filter((thread: MessageThread) => thread.linkedTo?.type === filterFeature)
    }

    // Sort by most recent activity
    return threads.sort(
      (a: MessageThread, b: MessageThread) => new Date(b.lastActivity).getTime() - new Date(a.lastActivity).getTime()
    )
  }, [messageThreads, searchQuery, filterUser, filterFeature, searchThreads])

  const getFeatureTypes = () => {
    const types = new Set(messageThreads.map((t: MessageThread) => t.linkedTo?.type).filter(Boolean))
  ...
  const getLinkedEntityBadge = (thread: MessageThread) => {
    ...
    return (
      <Badge
        variant="outline"
        className={cn("text-xs", typeColors[thread.linkedTo.type as keyof typeof typeColors] || typeColors["report"])}
      >
  ...
  const getLastMessagePreview = (thread: MessageThread) => {
    ...
    return lastMessage.content.length > 60 ? lastMessage.content.substring(0, 60) + "..." : lastMessage.content
  }
    ...
    return (
      <div className={cn("grid grid-cols-12 gap-6 h-full", className)}>
        ...
        <div className="col-span-8">
          <Card className="h-full">
            <CardContent className="flex items-center justify-center h-full">
              <div className="text-center text-muted-foreground">Select a message thread</div>
            </CardContent>
          </Card>
        </div>
      </div>
                ...
                {filteredThreads.map((thread: MessageThread) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground mb-2 line-clamp-2">{getLastMessagePreview(thread)}</p>

                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-1">
                          <Users className="w-3 h-3 text-muted-foreground" />
                          <div className="flex -space-x-1">
                            {thread.participants.slice(0, 3).map((participantId: string) => (
                              <Avatar key={participantId} className="w-4 h-4 border border-background">
                                <AvatarFallback className="text-[8px]">{getUserInitials(participantId)}</AvatarFallback>
                              </Avatar>
                            ))}
                        ...
                        <div className="flex items-center text-xs text-muted-foreground">
                          ...
                          {formatDistanceToNow(
                            typeof lastMessageTime === "string" ? parseISO(lastMessageTime) : lastMessageTime,
                            { addSuffix: true }
                          )}
                        </div>
                      </div>
