...
describe('TakeTourButton Page-Specific Functionality', () => {
  ...
  it('starts staffing tour when on staff-planning page', () => {
    mockPathname.mockReturnValue('/dashboard/staff-planning')
...
describe('Tour Auto-Launch Functionality', () => {
  ...
  it('auto-launches login tour on login page', async () => {
    ...
    render(<MockProviders><div>Test</div></MockProviders>)
...
describe('Enhanced Tour Anchoring', () => {
  ...
  it('ensures target element visibility before showing tooltip', async () => {
    ...
    jest.mocked(require('@/lib/tour-utils').tourDOMUtils.ensureElementVisibility).mockImplementation(mockEnsureElementVisibility)
  ...
  it('handles off-screen elements with scrolling', async () => {
    ...
    jest.mocked(require('@/lib/tour-utils').tourDOMUtils.scrollElementIntoView).mockImplementation(mockScrollElementIntoView)
    ...
    jest.mocked(require('@/lib/tour-utils').tourDOMUtils.isElementInViewport).mockImplementation(mockIsElementInViewport)
    jest.mocked(require('@/lib/tour-utils').tourDOMUtils.isElementPartiallyVisible).mockImplementation(mockIsElementPartiallyVisible)
  ...
  it('handles popover and dropdown interactions', async () => {
    ...
    jest.mocked(require('@/lib/tour-utils').tourDOMUtils.findElementWithFallbacks).mockImplementation(mockFindElementWithFallbacks)
...
describe('Tour Performance Optimization', () => {
  ...
  it('meets LCP target of <1s for tour initialization', async () => {
    ...
    jest.mocked(require('@/lib/tour-utils').tourPerformance.measureExecutionTime).mockImplementation(mockMeasureExecutionTime)
