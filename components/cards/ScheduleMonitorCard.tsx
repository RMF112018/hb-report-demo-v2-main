...
interface ProjectManagerData {
  ...
  variance: {
    ...
    costScheduleIndex: number
  }
  logicHealth: {
    score: number
    issues: number
    missingLogic: number
    danglingActivities: number
...
interface ProjectExecutiveData {
  scheduleHealth: number
  scheduleGrade: string
...
type ScheduleData = ProjectManagerData | ProjectExecutiveData | ExecutiveData | AdminData

// Type guards
function isProjectManagerData(data: ScheduleData): data is ProjectManagerData {
  return "totalActivities" in data && "variance" in data && "updateMetrics" in data
}

function isProjectExecutiveData(data: ScheduleData): data is ProjectExecutiveData {
  return "portfolioMetrics" in data && "totalProjects" in data
}

function isExecutiveData(data: ScheduleData): data is ExecutiveData {
  return "companyMetrics" in data && "totalProjects" in data
}

function isAdminData(data: ScheduleData): data is AdminData {
  return "totalActivities" in data && "variance" in data && "updateMetrics" in data
}
...
export function ScheduleMonitorCard({ card, config, span, isCompact, userRole }: ScheduleMonitorCardProps) {
  ...
  const getRoleBasedData = (): ScheduleData => {
    ...
    const baseData = {
      ...
      admin: {
        ...
        variance: {
          scheduleVariance: 3.7, // days ahead
          schedulePerformanceIndex: 1.12,
          costScheduleIndex: 1.08,
        },
        logicHealth: {
          score: 96.8,
          issues: 2,
          missingLogic: 0,
          danglingActivities: 1,
        ...
        delayAnalysis: {
          ...
          permitDelay: 0,
          designDelay: 0,
          recoveryPlan: "No recovery needed - ahead of schedule",
        },
        monthlyTrends: [
          { month: "Sep", health: 89.1, spi: 1.08, variance: 2.1, activities: 823, updates: 67 },
          { month: "Oct", health: 90.3, spi: 1.09, variance: 2.8, activities: 831, updates: 69 },
          { month: "Nov", health: 91.2, spi: 1.1, variance: 3.2, activities: 838, updates: 71 },
          { month: "Dec", health: 91.8, spi: 1.11, variance: 3.5, activities: 842, updates: 73 },
          { month: "Jan", health: 92.3, spi: 1.12, variance: 3.7, activities: 847, updates: 75 },
        ],
        milestones: [
          { name: "Infrastructure Upgrade", status: "completed", date: "2024-01-05", variance: 5 },
          { name: "Security Implementation", status: "in-progress", date: "2024-02-10", variance: 3 },
          { name: "System Migration", status: "upcoming", date: "2024-03-15", variance: 2 },
          { name: "Compliance Audit", status: "upcoming", date: "2024-04-20", variance: 1 },
        ],
        alerts: [
          { type: "success", message: "All systems running ahead of schedule", category: "Performance" },
          { type: "info", message: "Scheduled maintenance window approaching", category: "Maintenance" },
          { type: "warning", message: "Minor logic constraint needs review", category: "Logic" },
        ],
  ...
  // @ts-ignore - Complex role-based data structure with union types requires ignoring TypeScript errors
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "behind":
        return (
          <Badge className="bg-orange-100 dark:bg-orange-950/30 text-orange-800 dark:text-orange-200">Behind</Badge>
        )
      case "critical":
  ...
  const getLogicHealthScore = (data: ScheduleData): number => {
    if (isProjectManagerData(data) || isAdminData(data)) {
      return data.logicHealth.score
    } else if (isProjectExecutiveData(data)) {
      return data.logicHealth.avgScore
    } else if (isExecutiveData(data)) {
      return data.logicHealth.companyScore
    }
    return 80
  }

  const getUpdateMetricsScore = (data: ScheduleData): number => {
    if (isProjectManagerData(data) || isAdminData(data)) {
      return data.updateMetrics.dataQuality
    }
    return 85
  }

  const getCriticalPathScore = (data: ScheduleData): number => {
    if (isProjectManagerData(data) || isAdminData(data)) {
      return data.criticalPath.health
    } else if (isProjectExecutiveData(data)) {
      return data.criticalPath.healthScore
    }
    return 78
  }

  const getVarianceScore = (data: ScheduleData): number => {
    if (isProjectManagerData(data) || isAdminData(data)) {
      return data.variance.scheduleVariance
    } else if (isProjectExecutiveData(data)) {
      return data.portfolioMetrics.avgVariance
    } else if (isExecutiveData(data)) {
      return data.companyMetrics.avgVariance
    }
    return -10
  }

  const getSPIScore = (data: ScheduleData): number => {
    if (isProjectManagerData(data) || isAdminData(data)) {
      return data.variance.schedulePerformanceIndex
    } else if (isProjectExecutiveData(data)) {
      return data.portfolioMetrics.avgSPI
    } else if (isExecutiveData(data)) {
      return data.companyMetrics.avgSPI
    }
    return 0.85
  }

  const getTotalCount = (data: ScheduleData): number => {
    if (isProjectManagerData(data) || isAdminData(data)) {
      return data.totalActivities
    } else if (isProjectExecutiveData(data) || isExecutiveData(data)) {
      return data.totalProjects
    }
    return 0
  }

  const getOnScheduleText = (data: ScheduleData): string => {
    if (isProjectManagerData(data) || isAdminData(data)) {
      return `${data.criticalPath.activities} acts`
    } else if (isProjectExecutiveData(data)) {
      return `${data.portfolioMetrics.onSchedule}/${data.totalProjects}`
    } else if (isExecutiveData(data)) {
      return `${data.companyMetrics.onSchedule}/${data.totalProjects}`
    }
    return "0/0"
  }
  ...
  const radarData = [
    {
      dimension: "Logic Health",
      value: getLogicHealthScore(data),
    },
    {
      dimension: "Progress Updates",
      value: getUpdateMetricsScore(data),
    },
    {
      dimension: "Critical Path",
      value: getCriticalPathScore(data),
    },
    {
      dimension: "Milestone Performance",
      value: Math.max(0, 100 + getVarianceScore(data) * 2),
    },
    {
      dimension: "Schedule Adherence",
      value: getSPIScore(data) * 100,
    },
  ]
  ...
  const onScheduleCount =
    role === "executive"
      ? (data as any).companyMetrics?.onSchedule || 12
      : role === "project-executive"
      ? (data as any).portfolioMetrics?.onSchedule || 4
      : (data as any).milestones?.filter((m: any) => m.status === "completed").length || 2

  const criticalCount =
    role === "executive"
      ? (data as any).companyMetrics?.criticallyBehind || 3
      : role === "project-executive"
      ? (data as any).portfolioMetrics?.criticallyBehind || 2
      : (data as any).logicHealth?.issues || 1

  return (
    <div className="relative h-full" data-tour="schedule-monitor-card">
        ...
        <div className="flex-shrink-0 p-2 sm:p-2.5 lg:p-1.5 sm:p-2 lg:p-2.5 border-b border-gray-200 dark:border-gray-600">
          <div className="flex items-center gap-2 mb-1 sm:mb-1.5 lg:mb-2">
            ...
            <div className="flex items-center gap-1 ml-auto">
              {getTrendIcon(getVarianceScore(data))}
          ...
          <div className="grid grid-cols-3 gap-1 sm:gap-1.5 lg:gap-2">
            ...
            <div className="text-center p-1.5 sm:p-2 lg:p-2.5 bg-gray-200 dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-700">
              <div className="font-bold text-lg text-blue-700 dark:text-blue-400">
                {(getSPIScore(data) * 100).toFixed(0)}
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="text-center">
              <div className="relative inline-block">
                <div className="w-16 h-16 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center border-3 border-gray-300 dark:border-gray-500">
                  <div className="text-center">
                    <div className={cn("text-lg font-bold", getHealthColor(data.scheduleHealth))}>
                      {data.scheduleHealth.toFixed(0)}
                    </div>
                    <div className="text-xs text-muted-foreground">Health</div>
                  </div>
                </div>
                <div className="absolute -bottom-1 -right-1">{getTrendIcon(getVarianceScore(data))}</div>
              </div>
              <div className="mt-1 text-xs text-muted-foreground">Grade: {data.scheduleGrade}</div>
            </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-xs font-semibold ${
                      getVarianceScore(data) > -5
                        ? "text-green-600 dark:text-green-400"
                        : getVarianceScore(data) > -15
                        ? "text-yellow-600 dark:text-yellow-400"
                        : "text-red-600 dark:text-red-400"
                    }`}
                  >
                    {getVarianceScore(data).toFixed(1)} days
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-xs font-semibold ${
                      getSPIScore(data) >= 0.95
                        ? "text-green-600 dark:text-green-400"
                        : getSPIScore(data) >= 0.85
                        ? "text-yellow-600 dark:text-yellow-400"
                        : "text-red-600 dark:text-red-400"
                    }`}
                  >
                    {getSPIScore(data).toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-muted-foreground">Logic Health</span>
                  <span className={`text-xs font-semibold ${getHealthColor(getLogicHealthScore(data))}`}>
                    {getLogicHealthScore(data).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="text-center p-2 bg-gray-200 dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-700">
                <div className="text-sm font-medium text-foreground">{getTotalCount(data)}</div>
                <div className="text-xs text-muted-foreground">
                  {role === "project-manager" ? "Activities" : "Projects"}
                </div>
              </div>
              <div className="text-center p-2 bg-gray-200 dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-700">
                <div className="text-sm font-medium text-foreground">{getOnScheduleText(data)}</div>
                <div className="text-xs text-muted-foreground">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white/10 dark:bg-black/10 rounded-lg p-2 sm:p-2.5 lg:p-1.5 sm:p-2 lg:p-2.5">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span>Variance:</span>
                      <span className="font-medium text-purple-300">{getVarianceScore(data).toFixed(1)} days</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Performance Index:</span>
                      <span className="font-medium text-yellow-300">{getSPIScore(data).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className={cn("font-medium", getHealthColor(data.scheduleHealth))}>
                        {data.scheduleHealth >= 85
                          ? "Excellent"
                          : data.scheduleHealth >= 70
                          ? "Good"
                          : data.scheduleHealth >= 55
                          ? "Fair"
                          : "Poor"}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-green-400">
                        {role === "project-manager"
                          ? isProjectManagerData(data) || isAdminData(data)
                            ? data.updateMetrics.frequency
                            : "Weekly"
