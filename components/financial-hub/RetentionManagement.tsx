...
import { Shield, DollarSign, Calendar, TrendingUp, CheckCircle, Clock, AlertTriangle, Building2, FileText } from "lucide-react";
...
export default function RetentionManagement({ userRole, projectData }: RetentionManagementProps) {
                      ...
                      <div key={index} className="flex items-center justify-between text-sm">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Released: {formatCurrency(item.released)}
                          </div>
                        </div>
                      </div>
                    ))}
                      ...
                      {retentionData.map((retention, index) => (
                        <tr key={index} className="border-b hover:bg-muted/50">
                          ...
                          <td className="text-right p-2 font-mono">
                            {formatCurrency(retention.totalContract)}
                          </td>
                          <td className="text-right p-2">
                            {(retention.retentionRate * 100).toFixed(1)}%
                          </td>
                          <td className="text-right p-2 font-mono">
                            {formatCurrency(retention.retentionHeld)}
                          </td>
                          <td className="text-right p-2 font-mono">
                            {formatCurrency(retention.retentionReleased)}
                          </td>
                          <td className="text-center p-2">
                            <Badge variant={
                              retention.status === "Active" ? "default" : 
                              retention.status === "Partial Release" ? "secondary" : "outline"
                            }>
                              {retention.status}
                            </Badge>
                          </td>
  ...
  return (
    <div className={`space-y-6 ${isFullscreen.retention ? 'fixed inset-0 z-[9999] bg-background p-6 overflow-auto' : ''}`}>
      {/* Controls Bar */}
      <div className="flex items-center justify-between">
        <ViewToggle />
        <FullscreenToggle
          isFullscreen={isFullscreen.retention}
          onToggle={() => toggleFullscreen('retention')}
        />
      </div>
          ...
          <CardContent className="pt-6">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-indigo-800 dark:text-indigo-200">Compliance Status</p>
                  <p className="text-xs text-indigo-700 dark:text-indigo-300">
                    100% compliance with standard 5% retention rate across all contracts
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-indigo-800 dark:text-indigo-200">Release Schedule</p>
                  <p className="text-xs text-indigo-700 dark:text-indigo-300">
                    {formatCurrency(44500)} pending release over next 3 months
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-indigo-700 dark:text-indigo-300">
                    Low risk with strong contractor performance records
                  </p>
                </div>
              </div>
            </div>
