...
export default function JCHRCard({ userRole, projectData }: JCHRCardProps) {
  ...
  const parseCostCode = (costCode: string) => {
    ...
    const category = costCode.includes('.MAT') ? 'Material' :
                    costCode.includes('.LAB') ? 'Labor' :
                    costCode.includes('.LBN') ? 'Labor Burden' :
                    costCode.includes('.SUB') ? 'Subcontract' :
                    costCode.includes('.OVH') ? 'Overhead' : 'Other';
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{formatCurrency(summaryMetrics.contractValue)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{summaryMetrics.profitMargin.toFixed(1)}%</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{summaryMetrics.financialHealth.toFixed(0)}%</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Total Cost to Date</p>
                <p className="text-lg font-semibold">{formatCurrency(summaryMetrics.totalActual)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Current Period Spend</p>
                <p className="text-lg font-semibold">{formatCurrency(summaryMetrics.currentPeriodSpend)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{summaryMetrics.percentSpent.toFixed(1)}%</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <TrendingUp className={`h-4 w-4 ${summaryMetrics.totalVariance >= 0 ? 'text-red-500' : 'text-green-500'}`} />
              <div>
                <p className="text-xs text-muted-foreground">Variance to Budget</p>
                <p className="text-lg font-semibold">
                  {summaryMetrics.totalVariance >= 0 ? '+' : ''}{formatCurrency(summaryMetrics.totalVariance)}
                </p>
              </div>
            </div>
            <Badge variant={summaryMetrics.totalVariance >= 0 ? "destructive" : "default"}>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>
                    <Button variant="ghost" size="sm" onClick={() => handleSort('costCode')} className="flex items-center">
                      Cost Code
                  ...
                  <TableHead className="text-right">
                    <Button variant="ghost" size="sm" onClick={() => handleSort('budgetAmount')} className="flex items-center">
                      Budget
                      <ArrowUpDown className="ml-1 h-3 w-3" />
                    </Button>
                  </TableHead>
                  <TableHead className="text-right">
                    <Button variant="ghost" size="sm" onClick={() => handleSort('actualCost')} className="flex items-center">
                      Actual Cost
                      <ArrowUpDown className="ml-1 h-3 w-3" />
                    </Button>
                  </TableHead>
                  ...
                  <TableHead className="text-right">
                    <Button variant="ghost" size="sm" onClick={() => handleSort('variance')} className="flex items-center">
                      Variance
                      <ArrowUpDown className="ml-1 h-3 w-3" />
                    </Button>
                  </TableHead>
                    ...
                    <TableRow className="bg-muted/50 font-semibold">
                      ...
                      <TableCell className="text-right">
                        <span className={group.totals.variance >= 0 ? "text-red-600" : "text-green-600"}>
                          {group.totals.variance >= 0 ? '+' : ''}{formatCurrency(group.totals.variance)}
                        </span>
                      </TableCell>
                      <TableCell className="text-right">
                        {group.totals.budget > 0 ? ((group.totals.actual / group.totals.budget) * 100).toFixed(1) : '0.0'}%
                      </TableCell>
                    </TableRow>

                    {/* Division Items */}
                    {expandedDivisions.has(group.division) && group.items.map((item) => {
                      const { category } = parseCostCode(item.costCode);
                      return (
                        <TableRow key={item.costCode}>
                          ...
                          <TableCell>
                            <Badge 
                              variant="outline" 
                              className={
                                category === 'Material' ? 'border-blue-200 text-blue-700' :
                                category === 'Labor' ? 'border-green-200 text-green-700' :
                                category === 'Labor Burden' ? 'border-yellow-200 text-yellow-700' :
                                category === 'Subcontract' ? 'border-purple-200 text-purple-700' :
